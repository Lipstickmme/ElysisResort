# Deploying Elysis Luxury Resort to Vercel
...
| `/api/*` (suites, experiences, reservations, chat, inbound mail) | One Vercel serverless function (`api/[...path].js`) |
| Reservation enquiries, chat transcripts, inbound mail | Supabase (Postgres) |
...
is chosen first when several are present.

| What | Names it looks for, best first | Suggested size |
| ---- | ------------------------------ | -------------- |
| Home hero, three slides | `elysis-hero-1`, `elysis-hero-2`, `elysis-hero-3` | 2400 x 1350 |
| Landing sections | `elysis-story`, `elysis-suites`, `elysis-experiences`, `elysis-dining`, `elysis-gallery`, `elysis-reserve` | 1920 x 1200 |
| Page headers | `elysis-suites-header`, `elysis-experiences-header`, `elysis-dining-header`, `elysis-gallery-header`, `elysis-reserve-header`, `elysis-careers-header` | 2400 x 1100 |
| Any of the above, fallback | `elysis1` ... `elysis8` | |
| A residence | `suite-<id>`, `villa-<id>`, `residence-<id>`, plus `-2`, `-3`, `-4` for the gallery and `-plan` for the floor plan | 1600 x 1200 |
| An experience | `experience-<id>`, plus `-2`, `-3` | 1600 x 1200 |
| A restaurant or bar | `dining-<id>`, plus `-2`, `-3` | 1600 x 2000 |
| A gallery frame | `gallery-<id>` | 1600 x 1200 |
| The host's portrait | `gm` | 900 x 1100 |

`public/assets/img/README.md` carries the same table, next to the files. The
exact list each slot accepts is in `src/data/images.json` for the page furniture,
and in the `prefer` array on each entry of `suites.json`, `experiences.json`,
`dining.json` and `gallery.json`.

The ids are the ones in those files: `suite-aegean-loft.webp`,
`villa-helios.webp`, `experience-bonfire.webp`, `dining-thalassa.webp`,
`gallery-bonfire.webp`, and so on.

Notes:

- Names are matched without regard to case, so `Elysis3.PNG` and `elysis3.png`
  both work. That matters: the deploy runs on Linux, where the two are
  different files.
- Keep files under about 400 KB. The header images are full-bleed on every
  interior page, so a 1.2 MB PNG is felt on every visit. Save a WebP copy beside
  the original (same name, `.webp`) and it is used in preference automatically.
- Type sits over the headers and the hero behind a dark scrim, so pick frames
  that stay readable when darkened. A bright sky behind white type is the usual
  regret.
- Run `npm run build` after adding files, or let Vercel do it on deploy. The
  build log prints which photographs it found, and says so plainly when it is
  still running on the drawn artwork.
- The brand marks in `public/assets/brand/` are SVG stand-ins drawn to match the
  supplied logo. Replace `elysis-mark.svg`, `elysis-mark-light.svg` and
  `elysis-wordmark.svg` with the real artwork (same names, same viewBox
  proportions) and the nav, the footer and the dashboard follow. The favicons are
  drawn by `npm run icons`; replace `public/favicon.svg`, `favicon.png` and
  `apple-touch-icon.png` directly if you would rather not.
...
2. Clone it, add your images, and check it locally:
   ...
   cd <your-repo>
   npm install
   # drop in photography, edit src/data/*.json
   npm run build && npm start        # http://localhost:3000
   git add -A && git commit -m "Add the resort photography" && git push
   ```
...
3. Leave the build settings alone. `vercel.json` already sets the build command,
   the `public` output directory, clean URLs and the `/contact` redirect, and
   declares `"framework": null`. Residence and experience pages need no rewrite:
   the build writes `public/suites/<id>.html`, and clean URLs serve it at
   `/suites/<id>`. That last one matters: `express` is a dependency,
...
2. Create the tables. In the Supabase dashboard open **SQL Editor, New query**, and
   ...
   | [`supabase/migrations/0003_reservations.sql`](../supabase/migrations/0003_reservations.sql) | The stay on an enquiry: `arrival`, `departure`, `nights`, `adults`, `children`, `suite_id`, `phone` |

   0003 is worth running but is not load-bearing: without it the site still
   takes bookings, writes the columns every version of the schema has, and
   repeats the dates in the message body. With it, the desk can sort by who
   arrives next.
...
The function logs `[elysis] storage: Supabase` on the next request. Confirm what the
...
## Step 6: Send reservation enquiries and chat to your inbox (Resend)
...
3. In Vercel, add:
   ...
   | `FORM_FROM` | `Elysis Website <onboarding@resend.dev>` to start |
...
`Elysis Luxury Resort <reservations@yourdomain.com>`. Mail from a verified domain is far less
...
This gives you `reservations@yourdomain.com` that lands in whatever inbox you actually read.
The endpoint is `POST /api/inbound/resend`.

1. In Resend, verify your domain if you have not already, and enable **inbound email**
   for the address you want (for example `reservations@yourdomain.com`), following Resend's
   DNS instructions.
