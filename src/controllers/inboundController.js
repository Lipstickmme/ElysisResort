...
async function fetchBody(emailId) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.warn('[elysis] could not fetch inbound body:', res.status, raw.slice(0, 300));
      return null;
    }
    let data;
    try {
      ...
      console.warn('[elysis] inbound body was not json:', raw.slice(0, 200));
      return null;
    }
    ...
    console.warn('[elysis] inbound body fetch error:', err.message);
    return null;
  }
}
...
exports.resend = async (req, res, next) => {
  try {
    ...
    if (!result.ok) {
      console.warn('[elysis] inbound webhook rejected:', result.reason);
    ...
    if (supabase) {
      try {
        ...
        console.error('[elysis] failed to file inbound email:', err.message);
      }
    }
